...
export default function CategoryFilter({ value, onChange }: Props) {
  ...
  const selected = value === "all" ? null : categories.find(c => c.slug === value);
  const displayName = value === "all" ? "Все категории" : (selected ? selected.name : "Категория");
        ...
        <div className="absolute top-full right-0 mt-1 w-64 max-h-80 overflow-y-auto rounded-xl border border-border-default bg-surface-1 shadow-lg z-50">
          <div className="p-1">
            <button
              ...
              className={"w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors " + (value === "all" ? "bg-surface-2 text-text-primary" : "text-text-secondary hover:bg-surface-2 hover:text-text-primary")}
            >
            ...
            {categories.map((c) => (
              <button
                ...
                className={"w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors " + (value === c.slug ? "bg-surface-2 text-text-primary" : "text-text-secondary hover:bg-surface-2 hover:text-text-primary")}
              >
