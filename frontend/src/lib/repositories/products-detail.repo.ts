...
export async function getProductDetail(productId: number, days: number): Promise<ProductDetail | null> {
  const productRes = await pool.query(`
    SELECT p.id::text, p.name, p.sku, p.barcode, p.cost_price::float,
      COALESCE(p.brand, 'YourFit') AS brand, p.weight_g, p.is_active,
      c.name AS category, c.slug AS category_slug, p.created_at::text
    FROM products p
  ...
  const avgPriceRes = await pool.query(`
    SELECT CASE WHEN SUM(quantity) > 0
      THEN (SUM(revenue) / SUM(quantity))::float ELSE 0 END AS avg_price
  ...
  const returnPct = (m.total_sold + m.total_returns) > 0
    ? (m.total_returns / (m.total_sold + m.total_returns)) * 100 : 0;
  ...
  const abcRes = await pool.query(`
    WITH pr AS (
      SELECT product_id, SUM(revenue)::float AS revenue FROM sales
    ...
    SELECT
      CASE WHEN total > 0 AND (cum - revenue) / total < 0.8 THEN 'A'
           WHEN total > 0 AND (cum - revenue) / total < 0.95 THEN 'B'
           ELSE 'C' END AS grade,
  ...
  const calcForPay = m.total_revenue - m.total_commission;
  const commPct = m.total_revenue > 0
    ? parseFloat((m.total_commission / m.total_revenue * 100).toFixed(1)) : 0;
  ...
  return {
    ...
    chart: chartRes.rows,
    inventory: {
      items: inventoryRes.rows, total_stock: totalStock,
    ...
    geography: { by_country: [], by_warehouse: [] },
    price_history: priceHistRes.rows,
    finance: {
      revenue: m.total_revenue, for_pay: calcForPay,
      commission: m.total_commission, logistics: m.total_logistics,
      penalty: 0, acquiring: 0, storage: 0, deduction: 0,
      acceptance: 0, return_logistics: 0, additional_payment: 0,
      returns_amount: m.returns_amount,
      avg_spp_pct: 0, avg_commission_pct: commPct,
