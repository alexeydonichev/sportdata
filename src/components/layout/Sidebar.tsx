...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  useEffect(() => {
    api.notifications()
      .then(res => setAlertCount(res.summary.critical + res.summary.warning))
      .catch(() => {});
  }, []);
  ...
  return (
    <aside className="w-56 h-full border-r border-border-subtle bg-surface-0 flex flex-col">
        ...
        {nav.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onClose} className={cn(
              ...
              {showBadge && (
                <span className="min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-accent-red text-white text-[10px] font-bold px-1">
      ...
      <div className="border-t border-border-subtle px-3 py-3 space-y-1">
        <Link href="/settings" onClick={onClose} className="flex items-center gap-3 rounded-md px-2.5 py-2 text-[13px] text-text-secondary hover:text-text-primary hover:bg-surface-2 transition-colors">
          <Settings className="h-4 w-4" strokeWidth={1.5} />
          Настройки
        </Link>
        <button onClick={handleLogout} className="w-full flex items-center gap-3 rounded-md px-2.5 py-2 text-[13px] text-text-secondary hover:text-accent-red hover:bg-surface-2 transition-colors">
