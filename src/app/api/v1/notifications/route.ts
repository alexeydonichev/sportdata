...
export async function GET(req: NextRequest) {
  try {
    ...
    // 3. Sales anomalies (current week vs avg of prev 4 weeks)
    const salesAnomalies = await pool.query(`
      ...
      weekly_avg AS (
        SELECT product_id,
          AVG(week_revenue)::float AS avg_revenue,
          AVG(week_qty)::float AS avg_qty
        FROM (
          SELECT product_id,
            DATE_TRUNC('week', sale_date) AS week,
            SUM(revenue) AS week_revenue,
            SUM(quantity) AS week_qty
          FROM sales
