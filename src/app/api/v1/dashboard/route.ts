...
export async function GET(req: NextRequest) {
  ...
  try {
    // Current + previous period in one query
    ...
    const totalRev = m.total_revenue || 1;
    const byMarketplace = byMp.rows.map((r: Record<string, number | string>) => ({
      ...r,
      share_pct: parseFloat(((Number(r.revenue) / totalRev) * 100).toFixed(1)),
    }));
