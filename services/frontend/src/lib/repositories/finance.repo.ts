...
export interface FinanceData {
  pnl: { gross_revenue: number; returns_amount: number; net_revenue: number; for_pay: number;
    commission: number; logistics: number; penalty: number; cogs: number; net_profit: number; };
  margins: { gross_margin: number; net_margin: number; commission_pct: number; logistics_pct: number; return_rate: number; };
  changes: Record<string, number>;
  weekly: { week: string; revenue: number; for_pay: number; commission: number; logistics: number; net_profit: number; }[];
  by_category: { category: string; slug: string; revenue: number; for_pay: number; commission: number; logistics: number; cogs: number; net_profit: number; units: number; }[];
  warnings: string[];
}
...
export async function getFinance(days: number, category?: string, marketplace?: string): Promise<FinanceData> {
  ...
  // Penalty from returns table
  const penRes = await pool.query(`
    SELECT COALESCE(SUM(r.penalty),0)::float AS penalty
    FROM returns r JOIN products p ON p.id=r.product_id LEFT JOIN categories c ON c.id=p.category_id
    WHERE r.return_date>=CURRENT_DATE-$1::int ${catF}
  `, params);
  const penalty = penRes.rows[0].penalty;
  ...
  return {
    pnl: { gross_revenue:m.gross_revenue, returns_amount:m.returns_amount, net_revenue:m.net_revenue,
      for_pay:m.for_pay, commission:m.commission, logistics:m.logistics, penalty,
